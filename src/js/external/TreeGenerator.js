...
*/
export function createTree (trunkHeight, trunkTaper, trunkSlices, trunkMaterial, boughs, forks, forkAngle, forkRatio, branches, branchAngle, bowFreq, bowHeight, leavesOnBranch, leafWHRatio, leafMaterial, scene) {
    ...
    var _leaf = MeshBuilder.CreateDisc("leaf", {radius: _leaf_width/2, tessellation:12, sideOrientation:Mesh.DOUBLESIDE}, scene );
    ...
    var _set_mini_trees = function(particle, i, s) {
        ...
        var mini_top = new Vector3(_base.paths[_a][_base.paths[_a].length - 1].x, _base.paths[_a][_base.paths[_a].length - 1].y, _base.paths[_a][_base.paths[_a].length - 1].z);
        var turn = _turns[s];
        ...
        particle.scale = new Vector3(Math.pow(trunkTaper, boughs + 1), Math.pow(trunkTaper, boughs + 1), Math.pow(trunkTaper, boughs + 1));

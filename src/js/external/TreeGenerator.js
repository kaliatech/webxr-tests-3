...
var coordSystem = function (vec3) {
  ...
  if (Math.abs(vec3.x) == 0 && Math.abs(vec3.y) == 0) {
    var _x = new Vector3(vec3.z, 0, 0).normalize()
  } else {
    var _x = new Vector3(vec3.y, -vec3.x, 0).normalize()
  }
...
var createBranch = function (
  ...
  for (var d = 0; d < branchSlices; d++) {
    ...
    _core_point.addInPlace(
      branchSys.x.scale(bowHeight * Math.exp(-_d_slices_length) * Math.sin(bowFreq * _d_slices_length * Math.PI)),
    )
  ...
  var _branch = MeshBuilder.CreateRibbon('branch', { pathArray: _cross_section_paths, closeArray: true }, scene)
...
var createTreeBase = function (
  ...
  var _trunk_direction = new Vector3(0, 1, 0) //trunk starts in y direction

  var _branch

  var _trunk_sys = coordSystem(_trunk_direction)
  var _trunk_root_at = new Vector3(0, 0, 0)
  ...
  for (var f = 0; f < forks; f++) {
    ...
    _fork_branch_direction = _trunk_sys.y
      .scale(Math.cos(randPct(forkAngle, 0.15)))
      .add(_trunk_sys.x.scale(Math.sin(randPct(forkAngle, 0.15)) * Math.sin(_turn)))
      .add(_trunk_sys.z.scale(Math.sin(randPct(forkAngle, 0.15)) * Math.cos(_turn)))
    _fork_branchSys = coordSystem(_fork_branch_direction)
    _branch = createBranch(
      _top_point,
      _fork_branchSys,
    ...
    if (boughs > 1) {
      ...
      for (var k = 0; k < forks; k++) {
        ...
        _bough_direction = _fork_branchSys.y
          .scale(Math.cos(randPct(forkAngle, 0.15)))
          .add(_fork_branchSys.x.scale(Math.sin(randPct(forkAngle, 0.15)) * Math.sin(_bough_turn)))
          .add(_fork_branchSys.z.scale(Math.sin(randPct(forkAngle, 0.15)) * Math.cos(_bough_turn)))
...
export function createTree(
  ...
  var _leaf = MeshBuilder.CreateDisc(
    'leaf',
    { radius: _leaf_width / 2, tessellation: 12, sideOrientation: Mesh.DOUBLESIDE },
    scene,
  )

  //create solid particle system for leaves
  var _leaves_SPS = new SolidParticleSystem('leaveSPS', scene, { updatable: false })
  ...
  var _set_leaves = function (particle, i, s) {
    ...
    var _px =
      _base.paths[_a][_lower].x + ((_base.paths[_a][_upper].x - _base.paths[_a][_lower].x) * (_g - _gl)) / (_gu - _gl)
    var _py =
      _base.paths[_a][_lower].y + ((_base.paths[_a][_upper].y - _base.paths[_a][_lower].y) * (_g - _gl)) / (_gu - _gl)
    var _pz =
      _base.paths[_a][_lower].z + ((_base.paths[_a][_upper].z - _base.paths[_a][_lower].z) * (_g - _gl)) / (_gu - _gl)
    particle.position = new Vector3(
      _px,
      _py + ((0.6 * _leaf_width) / leafWHRatio + _base.radii[_a][_upper]) * (2 * (s % 2) - 1),
      _pz,
    )
  ...
  var _set_mini_trees = function (particle, i, s) {
    ...
    var mini_top = new Vector3(
      _base.paths[_a][_base.paths[_a].length - 1].x,
      _base.paths[_a][_base.paths[_a].length - 1].y,
      _base.paths[_a][_base.paths[_a].length - 1].z,
    )
    var turn = _turns[s]
    var _mini_direction = _mini_sys.y
      .scale(Math.cos(randPct(forkAngle, 0)))
      .add(_mini_sys.x.scale(Math.sin(randPct(forkAngle, 0)) * Math.sin(turn)))
      .add(_mini_sys.z.scale(Math.sin(randPct(forkAngle, 0)) * Math.cos(turn)))
    ...
    particle.scale = new Vector3(
      Math.pow(trunkTaper, boughs + 1),
      Math.pow(trunkTaper, boughs + 1),
      Math.pow(trunkTaper, boughs + 1),
    )
    particle.quaternion = Quaternion.RotationAxis(axis, _theta)
  ...
  var _set_branches = function (particle, i, s) {
    ...
    var _b = _places[s][1]
    var _mini_sys = _base.directions[_a]
    var _mini_place = new Vector3(_base.paths[_a][_b].x, _base.paths[_a][_b].y, _base.paths[_a][_b].z)
    _mini_place.addInPlace(_mini_sys.z.scale(_base.radii[_a][_b] / 2))
    var _turn = _bturns[s]
    var _mini_direction = _mini_sys.y
      .scale(Math.cos(randPct(branchAngle, 0)))
      .add(_mini_sys.x.scale(Math.sin(randPct(branchAngle, 0)) * Math.sin(_turn)))
      .add(_mini_sys.z.scale(Math.sin(randPct(branchAngle, 0)) * Math.cos(_turn)))
    var _axis = Vector3.Cross(Axis.Y, _mini_direction)
    var _theta = Math.acos(Vector3.Dot(_mini_direction, Axis.Y) / _mini_direction.length())
    particle.scale = new Vector3(
      Math.pow(trunkTaper, boughs + 1),
      Math.pow(trunkTaper, boughs + 1),
    ...
    particle.quaternion = Quaternion.RotationAxis(_axis, _theta)
  ...
  var _root = MeshBuilder.CreateBox('', {}, scene)
